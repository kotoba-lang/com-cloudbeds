(ns cloudbeds.main
  "Kotodama WASM entrypoint for the Cloudbeds clean-room actor (L4) — Clojure port.
  ...
  facts (`cloudbeds.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cloudbeds")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"cloudbeds.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cloudbeds-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cloudbeds-compat/ready)
